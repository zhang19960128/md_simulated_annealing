...
void PairBvv::settings(int narg, char **arg)
{
  ...
  power_global = force->numeric(FLERR,arg[0]);
...
/*  double epsilon_one = atof(arg[2]);*/
  ...
  double bvvs_one = atof(arg[4]);
  double bvvv0_one = atof(arg[5]);
...
/*          fread(&epsilon[i][j],sizeof(double),1,fp);
          ...
          fread(&bvvsparam[i][j],sizeof(double),1,fp);
          fread(&bvvv0[i][j],sizeof(double),1,fp);
...
void PairBvv::read_restart_settings(FILE *fp)
{
  ...
  if (me == 0) {
    ...
    fread(&power_global,sizeof(double),1,fp);
...
int PairBvv::pack_comm(int n, int *list, double *buf, int pbc_flag, int *pbc)
{
  int i,j,m;
...
void PairBvv::unpack_comm(int n, int first, double *buf)
{
  int i,m,last;
