...
void box::computelong(){
    ...
    double* allcharge=new double [size];
		...
		double chargei,chargej,temp,temp2,r3;
    for(size_t i=0;i<size;i++){
       ...
       for(std::list<int>::iterator j=allatom[i].neilj.begin();j!=allatom[i].neilj.end();j++){
         ...
         ShortRange+=1/epsil/4/pi*chargei*chargej/r*temp;
				 temp2=1.0/8.0/epsil/pi*chargei*chargej/r3*erfc(r/root2/sigma);
				 fx[i]=delx*temp2+fx[i];
				 fy[i]=dely*temp2+fy[i];
				 fz[i]=delz*temp2+fz[i];
				 temp=1.0/4/pi/epsil*1.0/2.0*chargei*chargej/rsq*(1.0/rootpi*2.0*exp(-1*rsq/2.0/sigma/sigma))*1.0/root2/sigma;
				 fx[i]=fx[i]+temp*delx;
				 fy[i]=fy[i]+temp*dely;
				 fz[i]=fz[i]+temp*delz;
			 }
